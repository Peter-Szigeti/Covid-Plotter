...
'''
Útmutatás a programhoz:tedd be egy mappába és futtasd le:)
...
def scrapeUrl(url):
    ...
    for i in range(1,getLastPage(url)):
        ...
        mylist.append(temp)
    df = pd.concat(mylist)
    df.to_csv('Data/covid19Deaths_scraped.csv')

def downloadTable(url):
    print('Beginning file download with urllib2...')
    urllib.request.urlretrieve(url, 'Data/covid19HungaryStatistics.xlsx')
...
def dateTheScrape():
    df = pd.read_csv('Data\covid19Deaths_scraped.csv')
    dates = pd.read_excel('Data\covid19HungaryStatistics.xlsx')
    df['Date'] = 'hi'
...
def plot_deathcountPerAgeGroup(df):
    ...
    plt.savefig('Results/deathcountPerAgeGroup_{}.png'.format(df['Dátum'][0]),dpi=300)
